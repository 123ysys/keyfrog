...
dnl 
dnl Test for the SQLite3 library of a particular version (or newer)
dnl
dnl This macro takes only one optional argument, required version
dnl of SQLite3 library. If required version is not passed,
dnl 3.0.0 is used in the test of existance of SQLite3.
dnl
...
[
    AC_ARG_WITH([sqlite3],
        AC_HELP_STRING(
            [--with-sqlite3@<:@=ARG@:>@],
            [use SQLite3 library from a standard location (ARG=yes)
    ...
    if test "x$WANT_SQLITE3" = "xno"; then
        AC_MSG_ERROR([SQLite3 is required, no disable is possible (--with-sqlite3=no given)])
    fi
...
#  error SQLite version is too old
        ...
        if test "$success" = "yes"; then
            ...
            AC_DEFINE(HAVE_SQLITE3,[],[Sqlite is installed])
	    ax_sqlite3="yes"
        fi
    fi
])
